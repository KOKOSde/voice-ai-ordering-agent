...
class PaymentProcessor:
    ...
    async def _process_stripe_payment(
        self, payment_id: str, order_id: str, amount: float, customer_id: Optional[str]
    ) -> Dict[str, Any]:
        """
    ...
    async def _refund_stripe(
        ...
        try:
            ...
            return {"success": True, "refund_id": refund.id, "amount": amount}

        except Exception as e:
            logger.error(f"Stripe refund failed: {e}")
            return {"success": False, "error": str(e)}

    def _simulate_refund(self, payment_id: str, amount: float) -> Dict[str, Any]:
        """Simulate a refund."""
        payment = self._payments.get(payment_id)
