...
class LLMProcessor:
    ...
    def _extract_intent(self, user_input: str) -> Dict[str, Any]:
        ...
        if any(word in input_lower for word in ["menu", "what do you have", "options", "tell me about"]):
            ...
            # Extract category
            for category in ["pizza", "pasta", "appetizer", "dessert", "drink", "salad", "wine"]:
                if category in input_lower:
                    intent["category"] = category + "s" if not category.endswith("s") else category
        
        elif any(word in input_lower for word in ["i'll have", "i want", "i'd like", "can i get", "order", "add", "give me"]):
        ...
        elif any(word in input_lower for word in ["that's it", "that's all", "confirm", "done", "place order", "checkout", "pay"]):
            intent["type"] = "confirm_order"
            intent["action"] = "confirm"
        
        elif any(word in input_lower for word in ["what's in my order", "my order", "what did i order", "order so far"]):
        ...
        if quantity_match:
            quantity_map = {"one": 1, "two": 2, "three": 3, "four": 4, "five": 5, "six": 6}
    ...
    def _extract_items(self, text: str) -> List[str]:
        ...
        menu_items = [
            "margherita", "pepperoni", "quattro formaggi", "meat lovers",
            "vegetarian", "hawaiian", "bbq chicken", "truffle mushroom",
            "spaghetti", "fettuccine", "lasagna", "carbonara", "penne",
            "bruschetta", "calamari", "caprese", "garlic bread", "mozzarella sticks",
            "caesar salad", "house salad", "tiramisu", "cannoli", "gelato",
            "pizza", "pasta", "salad", "soup", "wine", "beer", "soda"
        ]
    ...
    async def _generate_openai(self, prompt: str) -> str:
        ...
        try:
            ...
            response = await client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "You are a friendly restaurant phone ordering assistant."},
    ...
    async def analyze_sentiment(self, text: str) -> Dict[str, Any]:
        ...
        positive_words = ["great", "thanks", "perfect", "love", "awesome", "delicious", "wonderful"]
        negative_words = ["bad", "wrong", "upset", "angry", "terrible", "horrible", "awful"]
